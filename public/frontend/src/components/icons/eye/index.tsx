...
interface Props {
    ...
    onClick?: () => void;
    invisible: boolean;
    // invisible must start at true
}

function Eye (props: Props) {
    const wrapper = useRef<HTMLDivElement>(null);
    const handleClick = () => {
        if (wrapper.current!.classList.contains(styles.visible)) {
            wrapper.current!.classList.remove(styles.visible);
            wrapper.current!.classList.add(styles.invisible);
        } else {
            wrapper.current!.classList.remove(styles.invisible);
            wrapper.current!.classList.add(styles.visible);
        }
        if (props.onClick) props.onClick();
    }
    return (
    <div ref={wrapper} className={`${styles.eye} ${props.className ? props.className : ''}`} onClick={handleClick}>
        {!props.invisible ? (<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
